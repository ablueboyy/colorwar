...
1. 大廳先**選出戰砲台**：從所有砲台裡挑 8 種帶上場（兩位玩家各自選、互不影響），並**選地圖**（開房／單機者決定，加入者沿用該房地圖；岩石障礙不可蓋塔、不可染色、會擋子彈）
2. 一邊點 **Create Room** → 得到 4 碼房號
3. 另一邊開同一網址 → 選好砲台 → 輸入房號 → **Join Room**
4. 自己一個人測試：開兩個瀏覽器分頁（或一個正常 + 一個無痕視窗）

操作：

- **左鍵**：在自己顏色的地板上放砲台（只能放帶上場的 8 種）
- **左鍵點自己的砲台**：選取，砲台上方浮出升級／賣出按鈕
- **右鍵**：賣掉砲台（返還部分金錢）
